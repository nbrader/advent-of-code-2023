...
readWorld = undefined
  where readChar2Ds :: [Char] -> [((Int,Int), Char)]
...
combineWorlds w1 w2
    = w1 { worldLayers = M.unionWithKey combineLayers (worldLayers w1) (worldLayers w2),
           worldPoints = M.unionWithKey combinePoints (worldPoints w1) (worldPoints w2) }
  where combineLayers :: Char -> Points -> Points -> Points
        combineLayers = undefined
        
        combinePoints :: Char -> SinglePoint -> SinglePoint -> SinglePoint
